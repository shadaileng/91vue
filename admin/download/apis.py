...
class Download(object):
    ...
    async def download_status(self, filename, speed={'beg_t': 0, 'beg_d': 0, 'dtl': 0}):
        ...
        while True:
            if asyncio.current_task().cancelled():
                logging.warning(f'[task Stoping]cancelled.')
                break
            msg_result = loaddata(status_msg)
            if msg_result and not msg_result.get('_status', True):
                logging.warning(f'[task Stoping]{status_msg}:{msg_result}')
                break
            dumpdata(status_file, self.result)
            flag = True
            status_str = ''
    ...
    async def download_task(self, session, *, headers={}, chunk_size=1024):
        while not self.tasks.empty():
            ...
            while retry > 0:
                try:
                    ...
                    logging.error(f'[Timeout: {task["name"]}, remain: {retry - 1}]line: {sys.exc_info()[2].tb_lineno} status: {e.status}, message: {e.message}')
                except aiohttp.ClientResponseError as e:
                    logging.error(f'[ResponseError: {task["name"]}, remain: {retry - 1}]line: {sys.exc_info()[2].tb_lineno} status: {e.status}, message: {e.message}')
                except aiohttp.ClientError as e:
                    logging.error(f'[ClientError {task["name"]}, remain: {retry - 1}]line: {sys.exc_info()[2].tb_lineno}]: {e}')
                except ConnectionResetError as e:
                    logging.error(f'[ConnectionResetError, remain: {retry - 1}]line: {sys.exc_info()[2].tb_lineno} {e}')
    ...
    async def get_resource_size(self, url, *, headers={}):
        ...
        try:
            ...
            msg = f"[ServerDisconnectedError]head {url}: {e}"
            logging.error(msg)
        except aiohttp.ServerTimeoutError as e:
            msg = f'[Timeout]head {url}: {e}'
            logging.error(msg)
        except ConnectionResetError as e:
            msg = f'[ConnectionResetError]head {url}: {e}'
            logging.error(msg)
    ...
    async def download_file_range(self, url, filename):
        ...
        while retry > 0:
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=self.set_timeout(), trust_env=True) as session:
                tasks = [self.download_task(session, headers=headers) for index in range(self.thread_num)]
