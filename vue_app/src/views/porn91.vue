...
import { porn91 } from "@/store/porn91";
...
const porn91Store = porn91();
const { page, list, tabName, navigate, req, status } = storeToRefs(porn91Store);
...
const getlist = async (params) => {
    const _params = JSON.parse(JSON.stringify(porn91Store.getReqParams()))
    porn91Store.updateReqParams(params)
    if (!roles.value.includes('admin')) {
        porn91Store.setList([])
        porn91Store.setNavigate()
        return
    }
    router.push({
        path: '/',
        query: porn91Store.getReqParams(),
        hash: route.hash,
    })
    const params_ = JSON.parse(JSON.stringify(porn91Store.getReqParams()))
    let flag = true
...
const init = async (params) => {
    ...
    const res = await proxy.$api.get91List(porn91Store.getReqParams());
    if (res && res.code === 0) {
        porn91Store.setList(res.data.items)
        porn91Store.setPage(res.data.page)
...
const doLoadrflist = async (params) => {
    if (params.range.indexOf(':') <= 0) params.range = '1:2'
    const res = await proxy.$api.doLoadrflist(params);
    if (res) {
        if (res.code === 0) {
            proxy.$toast(res.msg)
        } else {
...
const doLoadAccuntlist = async (params) => {
    if (!params.uname) {
        proxy.$toast('uname 不能为空')
        return
    }
    if (params.range.indexOf(':') <= 0) params.range = '1:2'
    const res = await proxy.$api.doLoadAccuntlist(params);
    if (res) {
        if (res.code === 0) {
            proxy.$toast(res.msg)
        } else {
...
const stopTask = async (params) => {
    ...
    if (res) {
        if (res.code === 0) {
            proxy.$toast(res.msg)
            modalSlots.opt = {'items': res.data ? res.data['items'] : [] }
        } else {
            proxy.$toast(`请求异常: ${res.msg}`)
        }
    }
...
const optlist = reactive({
    ...
    */
    ...
    'loadrflist': {
        ...
        show: computed(()=>{return editable.value}),
        click: ()=>{
            modalSlots.item.loadrflist = true
            modalSlots.opt.range = "1:2"
            modal_.value.show()
        },
        submit: ()=>{
            doLoadrflist({range: modalSlots.opt.range})
        }
    },
    'loadaccuntlist': {
        name: '用户视频',
        ...
        click: ()=>{
            ...
            modalSlots.opt.params = {range: "1:2", uname: ''}
            modal_.value.show()
        },
        submit: ()=>{
            doLoadAccuntlist(modalSlots.opt.params)
        }
    },
    'update': {
        name: '修改状态',
        ...
        click: ()=>{
            ...
            // doVideoFun(proxy.$api.doLoadVideoPage, {items})
        },
        submit: ()=>{
    ...
    'downloadsrc': {
        ...
        click: ()=>{
            const items = getItems(selectItems)
            doVideoFun(proxy.$api.doDownloadSrc, {items})
        }
    },
    'check_keys': {
        name: '检查视频',
...
const modal_submit = ()=>{
    ...
    if (modalSlots.item.loadrflist) {
        optlist['loadrflist']['submit']()
    }
...
<template>
    ...
    <modal ref="modal_" :submitShow="modalSlots.item.import || modalSlots.item.update || modalSlots.item.loadrflist || modalSlots.item.loadaccuntlist" @submit="modal_submit" @hiden="hiden">
        ...
        <slideshow ref="slideshow_" v-show="modalSlots.item.video" v-slot="{ item, index }" :items="list" @select="(item)=>{play(item)}" >
            ...
            <div style="width: 100px; height: 60px;">
                <div v-loadback="{val1: item.poster, val2: item.poster_local !== null && `${(VUE_91_API + '/' + item.poster_local)}`, width: '95%'}" class="rounded img"></div>
            </div>
        ...
        <import91 v-if="modalSlots.item.import" v-model:content="modalSlots.opt.content" path='/root'></import91>
        <config
        ...
        <div class="container"
            v-if="modalSlots.item.loadrflist">
            <!-- doLoadrflist({range}) -->
            <div class="row configLine"></div>
            <div class="row">
                <div class="col-4">loadrflist</div>
                <input class="col-6" v-model="modalSlots.opt.range">
            </div>
            <div class="row">
        ...
        <div class="container"
            ...
            <div class="row">
                <div class="col-4">loadrflist</div>
                <input class="col-4" v-model="modalSlots.opt.params.uname">
                <input class="col-2" v-model="modalSlots.opt.params.range">
            </div>
            <div class="row configLine"></div>
        </div>
        <div class="container"
