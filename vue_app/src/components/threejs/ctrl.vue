...
const options = ref({
    ...
    addCube: function(){
        ...
        cube.scale.set(seed, seed, seed)
        cube.position.set(~~(Math.random() * 100) - 50, seed, ~~(Math.random() * 20) - 10)
        cube.name = `cube_${this.num + 1}`
        scene.add(cube);
        this.num++
    },
    removeCube: function() {
        const cubeLast = scene.getObjectByName(`cube_${this.num}`)
        if (cubeLast instanceof Mesh) {
            scene.remove(cubeLast)
            this.num--
        }
    },
})
...
const initGui = () => {
    ...
    gui.add(options.value, "num").listen()
    gui.add(options.value, "addCube")
    gui.add(options.value, "removeCube")
    // 雾化
    gui.add(options.value, "fog").onChange(el=>{
        if (el) {
            scene.fog = new Fog(0xffffff, 0.1, 100)
        } else {
            scene.fog = null
        }
    })
    // 材质覆盖
    gui.add(options.value, "override").onChange(el=>{
        if (el) {
