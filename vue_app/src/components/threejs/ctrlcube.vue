...
import { BufferAttribute, Euler, Quaternion } from 'three';
...
const cubeGeometry = new BoxGeometry(4, 2, 8)
...
window.addEventListener("deviceorientation", function(event) {
    ...
    // info.innerHTML = "Z轴: " + event.alpha + "</br>X轴: " + event.beta + "</br>Y轴: " + event.gamma;
    let alpha=-Math.ceil(event.alpha);
    let beta=Math.ceil(event.beta);
    let gamma=Math.ceil(event.gamma);
    var euler = new Euler();
    // 根据 alpha, beta, gamma 值计算欧拉角
    euler.set(beta * Math.PI / 180, alpha * Math.PI / 180, -gamma * Math.PI / 180, 'YXZ');

    // 将欧拉角转换为three.js中的四元数
    ...
    // alpha = fixdeg(alpha, 360)
    // beta = fixdeg(beta, 90)
    // gamma = fixdeg(gamma, 90)
